...
line("   WHAT'S LOADED")
empty()
line(f"     sessions   {SESSIONS:<4}        last       {LAST_DATE}")
line(f"     exemplars  {EXEMPLARS:<4}        canon      {CANON_TOTAL} entries")
line(f"     upgrades   {UP_TOTAL:<4}")
empty()
line(f"     tiers      {NB} no-brainer · {NM} normal · {PR} profound")
line(f"                {OL} outlandish")
empty()
line( "     topics     distributed systems · DDD · refactoring · ops")
empty()
