...
| [upgrade](.claude/skills/upgrade/SKILL.md) | `/upgrade <thought>` | **manual** | Capture a profound / novel / creative R&D idea into [upgrades/](upgrades/) — fast, formatted, filed. Decides tier (no-brainer / normal / outlandish / profound), generates meta + state tables, writes the entry. Migrated from `.claude/commands/upgrade.md` on 2026-05-17. | Bash subset, Read, Write |
