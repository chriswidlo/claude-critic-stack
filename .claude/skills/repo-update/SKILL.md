...
| # | Check | Canonical rule source | How to detect | Action available |
|---|---|---|---|---|
| **U17** | F2 fail-safe retirement condition status — should retire when comparator schema gains `refused` value | [CLAUDE.md](CLAUDE.md) "Things you must not do" F2 bullet + [garden/heirloom/2026-05-16-eval-context-disclosure/](garden/heirloom/2026-05-16-eval-context-disclosure/) | Check the garden entry's state-lifecycle table; if past `🔨 implemented`, surface for F2 rule retirement | Informational; operator removes F2 from CLAUDE.md when ready |
